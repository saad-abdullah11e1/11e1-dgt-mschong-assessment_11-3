...
class Game:
    """Game Class that stores all variables."""
    ...
    def __init__(self, root: tk.Tk, name):
        ...
        for x in range(self.CELLX):
            for y in range(int(self.CELLY)):
                ...
                self.canvas.create_rectangle(
                    x1, y1, x2, y2, fill="#DDD", outline="#FFFFFF"
                )

        self.snake = []

        (x1, y1, x2, y2) = self.get_cell_coord(
            int(self.CELLX / 2), int(self.CELLY / 2)
        )
        self.snake.append(
            self.canvas.create_rectangle(
                x1, y1, x2, y2, fill="#00FF00", outline="#00FF00"
            )
        )

        (x1, y1, x2, y2) = self.get_cell_coord(
            int(self.CELLX / 2) - 1, int(self.CELLY / 2)
        )
        self.snake.append(
            self.canvas.create_rectangle(
                x1, y1, x2, y2, fill="#00FF00", outline="#00FF00"
        ...
        (x1, y1, x2, y2) = self.get_cell_coord(
            int(self.CELLX / 2) - 2, int(self.CELLY / 2)
        )
        self.snake.append(
            self.canvas.create_rectangle(
                x1, y1, x2, y2, fill="#00FF00", outline="#00FF00"
        ...
        (x1, y1, x2, y2) = self.get_cell_coord(
            random.randint(0, self.CELLX - 1),
            random.randint(0, self.CELLY - 1),
        )
        self.apple = self.canvas.create_rectangle(
            x1, y1, x2, y2, fill="#FF0000", outline="#FF0000"
        )
    ...
    def change_direction(self, x, y):
        ...
        if len(self.new_directions) != 0:
            if (
                -x == self.new_directions[0][0]
                or -y == self.new_directions[0][1]
            ):
                return
        else:
            if -x == self.direction[0] or -y == self.direction[1]:
                return
    ...
    def game_loop(self):
        ...
        self.canvas.move(
            ...
            self.direction[1] * self.CELL_SIZE,
        )
        ...
        if self.canvas.coords(self.snake[0]) == self.canvas.coords(self.apple):
            ...
            while [x1, y1, x2, y2] in [
                self.canvas.coords(part) for part in self.snake
            ]:
                (x1, y1, x2, y2) = self.get_cell_coord(
                    random.randint(0, self.CELLX - 1),
                    random.randint(0, self.CELLY - 1),
